...
def train_loop(dataloader, model, avg_model, optimizer, epoch, device, scheduler=None, 
                # HELIP
                use_helip=False,      
...
def get_optimizer_and_scheduler(model, cfg):
    ...
    if CFG["training"]["warmup_epochs"] > 0:
        warmup_scheduler = torch.optim.lr_scheduler.LinearLR( # TODO: Explore better warmups
            optimizer, 
            start_factor=0.01,
            end_factor=1.0,
...
def save_checkpoint(model, avg_model, optimizer, scheduler, epoch, accuracy, best_acc, checkpoint_dir, is_best=False):
...
def train_cocoop(cfg, device):
    ...
    if CFG["validation"]["ema"]["enabled"]:
        avg_model = AveragedModel(custom_model,
