...
def get_data(data_dir="./data", train_transform=None, test_transform=None, mask_transform=None, segmentation_training=False):
    LOGGER.info(
    ...
    try:
        ...
        LOGGER.info(f"Dataset loaded: {len(train_set)} train, {len(val_set)} val, {len(test_set)} test")
...
def base_novel_categories(dataset):
    all_classes = set(dataset._labels) if hasattr(dataset, '_labels') else set(range(102))
    ...
    LOGGER.info(f"Split {num_classes} classes: {len(base_classes)} base, {len(novel_classes)} novel")
...
def split_data(dataset, base_classes):
    ...
    LOGGER.info(f"Split dataset: {len(base_dataset)} base samples, {len(novel_dataset)} novel samples")
...
def create_few_shot_dataset(dataset, shots_per_class=10):
    ...
    for class_label, indices in class_indices.items():
        ...
        sampled_indices = random.sample(
            indices, 
            min(shots_per_class, len(indices))
        )
        few_shot_indices.extend(sampled_indices)
    
    few_shot_dataset = Subset(dataset, few_shot_indices)
    
    LOGGER.info(f"Created {shots_per_class}-shot dataset: {len(few_shot_dataset)} samples")
    return few_shot_dataset

# Create data loaders for training and evaluation
def create_data_loaders(
    train_set, val_set, test_base, test_novel, 
    batch_size=32, test_batch_size=64, num_workers=4, pin_memory=True
):
    ...
    LOGGER.info(f"Created data loaders with batch_size={batch_size}, test_batch_size={test_batch_size}")
...
def zero_shot_evaluation(model, data_loader, class_names, class_indices=None, text_templates=None):
    ...
    if class_indices is not None:
        index_mapping = {orig_idx: eval_idx for eval_idx, orig_idx in enumerate(class_indices)}
    else:
        index_mapping = None
    ...
    with torch.no_grad():
        for images, targets in tqdm(data_loader, desc="Zero-shot evaluation"):
            ...
            if index_mapping is not None:
                mapped_targets = torch.tensor([index_mapping[t.item()] for t in targets], device=DEVICE)
            else:
...
class PromptLearner(nn.Module):
    def __init__(self, clip_model, classnames, n_ctx=4, ctx_init="", class_token_position="end", csc=False):
        super().__init__()
        ...
        if ctx_init:
            ...
            if csc:
                ...
                ctx_vectors = torch.empty(n_cls, n_ctx, ctx_dim, dtype=clip_model.dtype, device=DEVICE)
            else:
                LOGGER.info("Initializing a generic context")
                ctx_vectors = torch.empty(n_ctx, ctx_dim, dtype=clip_model.dtype, device=DEVICE)
...
class CoOp(nn.Module):
    def __init__(self, classnames, clip_model, n_ctx=4, ctx_init="", class_token_position="end", csc=False, 
        ...
        if CFG["promptsrc"]["enabled"] and CFG["promptsrc"]["prompt_ensemble"]["enabled"]:
            ...
            LOGGER.info(f"PromptSRC ensemble enabled: window_size={window_size}, sigma={sigma}")
        else:
...
def train_coop(model, train_loader, optimizer, criterion, epoch, avg_model=None, 
    ...
    if use_promptsrc:
        ...
        LOGGER.info(f"PromptSRC enabled: lambda_ma={lambda_ma:.4f}, lambda_td={lambda_td:.4f}")
    else:
        ...
        try:
            ...
            with torch.amp.autocast("cuda"):
                ...
                if use_helip and hard_pair_miner is not None and len(hard_pairs) > 0:
                    ...
                    batch_end = min(batch_start + train_loader.batch_size, len(dataset_indices))
                ...
                loss = contrastive_loss + lambda_hnml * hnml_loss + lambda_ma * ma_loss + lambda_td * td_loss
            ...
            if use_helip:
                total_hnml_loss += hnml_loss.item() if isinstance(hnml_loss, torch.Tensor) else 0.0
            if use_promptsrc:
                total_ma_loss += ma_loss.item() if isinstance(ma_loss, torch.Tensor) else 0.0
                total_td_loss += td_loss.item() if isinstance(td_loss, torch.Tensor) else 0.0
    ...
    if use_helip and use_promptsrc:
        ...
        LOGGER.info(f"Epoch {epoch + 1} - Training Loss: {avg_loss:.4f}, Accuracy: {avg_acc:.2f}%")
...
def create_segmentation_model():
    ...
    size_transform = torchvision.transforms.CenterCrop(CFG["bg_masking"]["training"]["image_size"])
...
def image_augmentation(mode, base_preprocess):
    ...
    if mode == "rotate_illumination":
        return transforms.Compose([
            ...
            transforms.Lambda(lambda img: TF.adjust_brightness(
                img, brightness_factor=np.random.uniform(1.1, 1.5)
            )),
            transforms.RandomResizedCrop(size=CFG["input"]["size"], scale=(0.7, 1.0)),
        ...
        return transforms.Compose([
            transforms.RandomRotation(degrees=30),
            transforms.Lambda(lambda img: TF.adjust_contrast(
...
def main():
    ...
    try:
        ...
        if augmentation_mode:
            LOGGER.info(f"Applying data augmentation: {augmentation_mode}")
        ...
        if use_helip:
            ...
            LOGGER.info(f"HELIP parameters: k={CFG['helip']['k']}, p={CFG['helip']['p']}, alpha={CFG['helip']['alpha']}, lambda_hnml={CFG['helip']['lambda_hnml']}")
        else:
        ...
        if use_promptsrc:
            ...
            LOGGER.info(f"PromptSRC parameters: lambda_ma={CFG['promptsrc']['mutual_agreement']['lambda_ma']}, lambda_td={CFG['promptsrc']['textual_diversity']['lambda_td']}")
            if CFG["promptsrc"]["prompt_ensemble"]["enabled"]:
                LOGGER.info(f"Prompt ensemble enabled: window_size={CFG['promptsrc']['prompt_ensemble']['window_size']}, sigma={CFG['promptsrc']['prompt_ensemble']['sigma']}")
        else:
